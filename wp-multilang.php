...
function wpm_activation_admin_notices() {
	...
	if ( get_transient( 'wpm_admin_notice_activation' ) ) {
		...
		$wpm_ui_link_text = ( strpos( $screen->id, 'wpm' ) === false ) ? sprintf( esc_html__( '<a href="%s">Customize your settings &rarr;</a>', 'wp-multilang' ), admin_url( 'admin.php?page=wpm-settings' ) ) : '';
		
		echo '<div class="updated notice is-dismissible"><p>' . esc_html__( 'Thank you for installing <strong>WP Multilang!</strong> ', 'wp-multilang' ) . $wpm_is_ready . $wpm_ui_link_text . '</p></div>';
